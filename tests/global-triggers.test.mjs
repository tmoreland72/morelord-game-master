...
test('global triggers populate new worlds for every GM, repair empty catalogs, and reject player writes',async()=>{
  ...
  try {
    ...
    await initializeGlobalTriggers();assert.equal(board.triggers.length,5);assert.ok(board.triggers.every(t=>!t.enabled));
    game.user=player;assert.throws(()=>saveGlobalTriggers([{id:'unauthorized'}]),/Only GMs/);assert.equal(file.triggers.length,5);
    const secondGM={id:'second-gm',isGM:true,active:true};game.users.set(secondGM.id,secondGM);game.user=secondGM;board={triggers:[]};
    await initializeGlobalTriggers();assert.equal(board.triggers.length,5);assert.ok(board.triggers.every(t=>!t.enabled));
