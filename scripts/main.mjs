...
export async function addTrigger({name,actorId,itemId,tableId,tableUuid,kind="item",id,gameMinutes=10,realMinutes=1}) {
  ...
  const trigger={macroUuid:triggerMacroUuid(kind),...interval,sourceWorld:game.world?.id,id:id ?? foundry.utils.randomID(),name:name?.trim() || (kind === "world-clock" ? "World Clock" : kind === "sneak" ? "Sneak Attack" : table.name),actorId:kind === "item" ? actorId : null,itemId:kind === "item" ? itemId : null,tableUuid:table?.uuid,tableId:table?.id,kind,actorName:kind === "item" ? actor.name : null,itemName:item?.name,tableName:table?.name,enabled:true,createdBy:game.user.id};
  await persist(n=>{const existing=n.triggers.find(t=>id ? t.id===id : t.kind===kind && (kind !== "item" || (t.actorId===actorId && t.itemId===itemId && t.tableUuid===trigger.tableUuid)));if(existing){trigger.id=existing.id;if(id)trigger.enabled=existing.enabled;Object.assign(existing,trigger);}else n.triggers.push(trigger);});
