...
Fresh installations seed five managed triggers globally: Wild Magic Surge, Volatile Magic, Sneak Attack, Lucky Finds, and World Clock. Every new rule starts stopped. The 0.1.2 migration adds missing built-ins to existing installations without changing existing rule IDs, enabled states, or table overrides. Explicit deletions after this migration remain deleted. The Item Use macro remains a compendium template and only becomes a control-panel trigger when configured through the API; 0.1.4 removes the accidentally seeded unconfigured entry while preserving configured item rules. Lucky Finds needs its world table. Roll of Fate is a one-shot macro, not a background trigger. The catalog and saved start/stop choices are shared across worlds on the same installation.

Regression: run defaultTriggersCheck from scripts/testing/default-triggers.mjs through Core’s shared in-game runner in Dev1. It checks fresh and Volatile-only catalogs against installed Macro documents, confirms every runtime stays stopped, and restores the prior world board.
